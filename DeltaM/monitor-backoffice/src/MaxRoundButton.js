...
class MaxButton extends Component {
    ...
    handleFound() {
        fetch(`${API_URL}/count/round?sport=${this.state.sport}&country=${this.state.country}&league=${this.state.league}${this.state.max !== null && this.state.max !== "" ? `&max=${this.state.max}` : ''}${this.state.min !== null && this.state.min !== "" ? `&min=${this.state.min}` : ''}`, {}).then(res => res.json()).then(res => {
            this.setState({
    ...
    render() {
        ...
        return (
            <div className="container-fluid" style={{ display: "inline" }}>
                <FlatButton primary label="Round Series" onClick={this.handleOpen} icon={<ViewIcon />} />
                <Dialog
                    title="Round Series"
