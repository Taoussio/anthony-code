...
class MaxButton extends Component {
    ...
    apiTeam(sport, country, q) {
        fetch(`${API_URL}/match/teams?q=${q}&sport=${sport}&country=${country}`, {}).then(res => res.json()).then(res => {
            this.setState({
                teams: [
                    ...res.data.map(elem => elem.value).filter(elem => !!elem),
                ]
    ...
    handleFound() {
        ...
        fetch(`${API_URL}/count?sport=${this.state.sport}&country=${this.state.country}&team=${this.state.team}${this.state.max !== null && this.state.max !== "" ? `&max=${this.state.max}` : ''}${this.state.min !== null && this.state.min !== "" ? `&min=${this.state.min}` : ''}`, {}).then(res => res.json()).then(res => {
            this.setState({
    ...
    render() {
        ...
        return (
            <div className="container-fluid" style={{ display: "inline" }}>
                <FlatButton primary label="Max Series" onClick={this.handleOpen} icon={<ViewIcon />} />
                <Dialog
                    title="Max Series"
                    actions={this.state.sport && this.state.country && this.state.team && (this.state.max || this.state.min) ? (
                        [
                            <FlatButton
                                label="Found"
                                primary={true}
                                    ...
                                    <AutoComplete
                                        ...
                                        onUpdateInput={(searchText, choices, event) => {
                                            if (event.source === 'change')
                                                this.apiTeam(this.state.sport, this.state.country, searchText)
                                        }}
                                    />
                                </div>
                                <div>
